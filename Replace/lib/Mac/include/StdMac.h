...
/* 2010/11/26	王井锋			新增将原串转换为ASCII码后再计算			*/
/* 2011/01/21	王井锋			新增密钥按位取反						*/
/************************************************************************/
#ifndef _STDMAC_H
#define _STDMAC_H
#include "3des.h"
...
/*
* 3DES加密得到mac码(处理密钥取前16字节，密钥按字符串取反)
* @keys		密钥（密钥只取16字节）
* @inputs		明文
* @output		密文
* @length		Mac码长度
...
/*
* 3DES加密得到mac码(处理密钥取前16字节，密钥按位取反)
* @keys		密钥（密钥只取16字节）
* @inputs		明文
* @output		密文
* @isPadding	8整数倍是否补位
* @isASCII		是否将原串先转成ASCII码字符串
*/
void getMac3(const std::string& key,const std::string& input,  std::string& output, const int length = 8, bool isPadding = false,bool isASCII = true);
/*
* 3DES加密得到mac码
* @keys		密钥（密钥只取16字节）
* @inputs		明文
...
*@dest		目标串
*/
void convertBitToNegate(const char* source,char *dest);
